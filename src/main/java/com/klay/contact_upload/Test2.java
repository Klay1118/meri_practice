package com.klay.contact_upload;

import java.io.BufferedReader;
import java.io.BufferedWriter;
import java.io.FileReader;
import java.io.FileWriter;
import java.io.IOException;
import java.util.ArrayList;
import java.util.Random;
import java.util.UUID;

import org.json.JSONException;
import org.json.JSONObject;
import org.json.JSONArray;

/**
 * 直接将名字改为一个客户
 */
public class Test2 {

    public static void main(String[] args) {
        // 读取nameID.txt文件中的NAMEID字段（key）对应值（value）并存储
        ArrayList<String> list = new ArrayList<String>();
        BufferedReader brname;
        try {
            brname = new BufferedReader(new FileReader("C:\\Users\\sales\\Downloads\\docs\\" +
                    "contact\\test2\\contact-000017.json"));
            String sname = null;
            while ((sname = brname.readLine()) != null) {
                // System.out.println(sname);
                list.add(sname);// 将对应value添加到链表存储
            }
            brname.close();
        } catch (IOException e1) {
            // TODO Auto-generated catch block
            e1.printStackTrace();
        }
        // 读取原始json文件并进行操作和输出
        try {
            BufferedReader br = new BufferedReader(new FileReader(
                    "C:\\Users\\sales\\Downloads\\docs\\contact\\test2\\contact-000017.json"));// 读取原始json文件
            BufferedWriter bw = new BufferedWriter(new FileWriter(
                    "C:\\Users\\sales\\Downloads\\docs\\contact\\test2\\contact-000017-test.json"));// 输出新的json文件
            String s = null, ws = null;
            while ((s = br.readLine()) != null) {
                // System.out.println(s);
                try {
                    JSONObject dataJson = new JSONObject(s);// 创建一个包含原始json串的json对象
                    JSONArray features = dataJson.getJSONArray("data");// 找到contact的json数组
                    for (int i = 0; i < features.length(); i++) {
                        JSONObject info = features.getJSONObject(i);// 获取contact数组的第i个json对象
                        //JSONObject properties = info.getJSONObject("公司名称");// 找到properties的json对象
                        // String name = info.getString("公司名称");// 读取对象里的字段值
                        // System.out.println(name);
                        // String test = name.replaceAll(name, "C00991707 系统预置客户test");
                        String name = info.getString("姓名");
                        //String id = info.getString("联系人ID");
                        UUID uuid = UUID.randomUUID();
                        Random rand=new Random();
                        int nextInt = rand.nextInt(1000);
                        info.put("联系人ID",uuid);
                        info.put("公司ID","b77a35e1-8439-4531-939d-ba3a988cd849");
                        info.put("公司名称","C00991707 系统预置客户test");
                        info.put("姓名",name+"_00000"+nextInt);
                       // info.remove("公司名称");
                        // System.out.println(name);
                        // System.out.println(test);
                       /* properties.put("NAMEID", list.get(i));// 添加NAMEID字段
                        // properties.append("name", list.get(i));
                        System.out.println(properties.getString("NAMEID"));
                        properties.remove("ISO");// 删除ISO字段*/
                    }
                    ws = dataJson.toString();
                    //System.out.println(ws);
                } catch (JSONException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                }
            }

            bw.write(ws);
            // bw.newLine();

            bw.flush();
            br.close();
            bw.close();

        } catch (IOException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }

    }

}

